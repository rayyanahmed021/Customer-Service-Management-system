	...
	if (fp != NULL)
	{
		...
		do
		{
			fieldCount = fscanf(fp, "%d~%c~%30[^~]~%10[^~]~%8[^~]~%d~%lf~%30[^\n]",
				...
				accounts[count].login.accountName,
				accounts[count].login.loginName,
				accounts[count].login.password,
				&accounts[count].demographic.birth_year,
				&accounts[count].demographic.income,
				accounts[count].demographic.country);
